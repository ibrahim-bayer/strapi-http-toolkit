...
import { mockProducts, mockCategories, mockProductVariants, mockProductVariantTranslations } from '../mockup/mockData';
import { PopulateOptions } from '../../PopulateOptions';

/**
 * Example 1: Filter products by exact name match
 */
export const filterProductsByExactName = (name: string): FilterOptions<ProductModel> => {
 ...
 */
export const filterProductsByNameContains = (searchTerm: string): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
 ...
 */
export const filterProductsByPriceRange = (minPrice: number, maxPrice: number): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
 ...
 */
export const filterProductsByCategory = (categoryName: string): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
    categories: {
 ...
 */
export const filterProductsByNameAndCategory = (name: string, categoryName: string): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
    $and: [
      { name: { $containsi: name } },
 ...
 */
export const filterProductsByMultipleCategories = (categoryNames: string[]): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
 ...
 */
export const filterProductsByVariantName = (variantName: string): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
    variants: {
 ...
 */
export const filterProductsByVariantTranslation = (translationText: string): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
    variants: {
      translations: {
...
/**
 * Example 9: Complex filter with multiple nested conditions
 */
...
/**
 * Example 10: Filter products by document ID
 */
export const filterProductsByDocumentId = (documentId: string): FilterOptions<ProductModel> => {
  const filter: FilterOptions<ProductModel> = {
    documentId: { $eq: documentId }
  };
  
  console.log('Filter for document ID:');
  console.log(JSON.stringify(filter, null, 2));
  console.log('Serialized query string:', qs.stringify({ filters: filter }));
 ...
 */
export const filterVariantsByTranslation = (translationText: string): FilterOptions<ProductVariantModel> => {
 ...
 */
export const filterTranslationsByProductPrice = (minPrice: number): FilterOptions<ProductVariantTranslationModel> => {
